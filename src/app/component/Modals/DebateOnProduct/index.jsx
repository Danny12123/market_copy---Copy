...
const Debate = ({ isDebateOnProduct, setIsDebateOnProduct }) => {
  ...
  return (
    ...
    >
      ...
      ></div>
      <div className="rounded-lg w-full sm:w-[480px] h-fit p-5 bg-white dark:bg-slate-900  z-[1000]">
        ...
        <form onSubmit={submitOfferHandler} className="my-3">
          <label className="text-sm mb-1">Product cost 120.99</label>
          <div className="flex items-center gap-2">
            <Input
              className="my-2"
              placeHolder="How much can you adored"
              type="number"
              value={userOffer}
              onChange={(e) => setUserOffer(e.target.value)}
            />
            <button className="w-[60px] px-3 py-2 rounded-sm text-xs bg-green-500 hover:bg-green-600 text-[#fff]">
              Talk
            </button>
          </div>
        ...
        <div className="mt-7">
          ...
          <div className="flex justify-end items-end gap-3 mt-2">
            <button
              onClick={() => setIsDebateOnProduct(false)}
              className="w-[80px] px-2 py-2 rounded-lg border text-xs border-[#eee]  hover:border-[#777] dark:text-white text-[#000]"
            >
              Cancel
            </button>
            <button className="w-[80px] px-3 py-2 rounded-lg text-xs bg-green-500 hover:bg-green-600 text-[#fff]">
              Save
            </button>
          </div>
        </div>
